...
To obtain an initial performance datapoint, we benchmarked the standard padded
\texttt{"abc"} block over 64 iterations in simulation. The current result is
summarized in Table~\ref{tab:perf}.
...
\midrule
Total cycles for 64 hashes & 24\,065 & 355\,136 \\
Cycles per hash & 376 & 5\,549 \\
\bottomrule
\end{tabularx}
\caption{Current one-block benchmark result for the padded \texttt{"abc"} case,
measured in simulation cycles.}
\label{tab:perf}
\end{table}

This corresponds to an observed speedup of about 14.8x for the accelerator over
the software-only implementation on the Croc CPU. While this is only one
benchmark point, it already demonstrates that the project produces a measurable
system-level improvement and not merely a functional hardware addition.
