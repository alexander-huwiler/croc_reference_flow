...
int main() {
    ...
    // Driver-level negative-path check: this should return START_ERR, not hang.
    ...
    CHECK_ASSERT_EOC(2, (status & SHA256_ACCEL_STATUS_DONE_BIT_MASK) == 0u);
    ...
    for (int i = 0; i < 8; ++i) {
        CHECK_ASSERT_EOC(6 + i, digest[i] == sha256_two_block_digest[i]);
    }
